 ...
 */
export async function getRawUrl(gistId: string): Promise<string[]> {
  // 发起 GET 请求，并等待响应结果
 ...
 */
export async function getGistPostsContent(urls: string[]): Promise<{}> {
  return await Promise.all(urls.map((url) => AxiosUtil.get(url)));
}

export async function getWakaTimeUrl(url: string): Promise<string[]> {
