 ...
 */
export function isGistId(gistId?: string): string | false {
  return check(InputType.GistId, gistId);
}
 ...
 */
export function isURL(url?: string): string | false {
  return check(InputType.Url, url);
}
...
function check(type: InputType, urlOrGistId?: string) {
  let reg;
